 ...
 * </ul>
 *
 * <p><b>SaaS 注册方式</b>：SaaS 主模块通过 {@code @Bean} 或 {@code @Configuration}
 * 创建 {@code PlayEngine} 实例，注入 {@code @Qualifier("teslamateJdbc")} 的
 * {@code JdbcTemplate} 和 {@code @Qualifier("teslamateTx")} 的
 * {@code PlatformTransactionManager}。
 *
 * <p><b>Bridge 注册方式</b>：bridge 只有单数据源，直接注入默认 {@code JdbcTemplate} 和
 * {@code PlatformTransactionManager}。
 */
public class PlayEngine {
  ...
  /**
   * 主构造器：从 {@code JdbcTemplate} + {@code PlatformTransactionManager} 建专属实例。
   * SaaS 侧传入 {@code @Qualifier("teslamateJdbc")} / {@code @Qualifier("teslamateTx")}；
   * bridge 侧传入默认 bean（单数据源）。
   */
  public PlayEngine(JdbcTemplate jdbcTemplate, PlatformTransactionManager txManager) {
    JdbcTemplate own =
        new JdbcTemplate(
            Objects.requireNonNull(
                jdbcTemplate.getDataSource(), "jdbcTemplate has no DataSource"));
  ...
  public RunResult run(PlayDefinition play, long carId, LocalDateTime start, LocalDateTime end) {
    ...
    Map<String, Object> row =
        Objects.requireNonNullElse(
            readOnlyTx.execute(status -> queryFirstRow(play, carId, start, end)), Map.of());
  ...
  public record CarLabel(String name, String model, String trimBadging) {
    ...
    public String displayName() {
      ...
      return sb.isEmpty() ? "我的特斯拉" : sb.toString();
    }
