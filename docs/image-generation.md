...
输出结构化 JSON + `creative-prompt.md` 生图描述模板。
...
2. **Qwen-Image-2.0（DashScope 国际版）**——需注册，新用户 100 张免费
   - 注册：https://dashscope-intl.aliyuncs.com
   - 实测中文海报文字渲染极强的商用 API
   - 见下方 [Qwen API 调用示例](#qwen-image-调用示例)

3. **Gemini flash image**——需 GCP billing（无真正免费层）；有 billing 的海外玩家可用；国内玩家直接走 Seedream
  ...
  否 → 有豆包 / 即梦?
         ...
         否 → 中国大陆用户 / 想要 MCP 一键生图?
                ...
                否 → 想注册 DashScope 国际版?
                       ...
                       否 → 请有 ChatGPT 订阅的朋友帮生图（或等自己开通一种渠道）
                            没有任何生图渠道 = JSON 结构化结论本身已可直接使用
```
