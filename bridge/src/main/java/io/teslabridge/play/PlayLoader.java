...
import io.teslabridge.play.PlayDefinition.ComputeStep;
import io.teslabridge.play.PlayDefinition.ExprStep;
import io.teslabridge.play.PlayDefinition.LevelStep;
import io.teslabridge.play.PlayDefinition.LookupStep;
import io.teslabridge.play.PlayDefinition.OutputField;
import io.teslabridge.play.PlayDefinition.TemplateStep;
import io.teslabridge.play.PlayDefinition.Threshold;
 ...
 * 实例化）→ 手写 schema 校验（镜像 {@code plays/play.schema.json}，不引 JSON-Schema 运行时
 * 依赖）→ {@link PlaySqlGuard} → 表达式编译 → 模板 lint。
 *
 * <p>schema 文件 {@code plays/play.schema.json} 是贡献者侧的单一事实源（本地可用任意
 * JSON-Schema 工具自检）；本类的手写校验与之保持一致，任何改 schema 的 PR 必须同步两处。
 *
...
final class PlayLoader {
     ...
     * PLAY_COMPUTE_ERROR，必须在加载期拒掉（spec §5 的 Where 列已注明 card-only）。
     */
    static final Set<String> CARD_ONLY_BUILTINS =
            Set.of("car_name", "car_model", "window_label", "generated_at");
     ...
     * 覆盖 fill/filter/clip-path 等属性与 &lt;style&gt; 文本里的引用。
     */
    ...
    /**
     * @param dirName play 目录名（必须与 manifest name 一致 —— registry 以 name 为 key，
     *     目录名错配会让 PLAYS_DIR 覆盖语义混乱）
     * @param playYamlBytes play.yaml 原始字节（参与内容 SHA-256）
     * @param cardTemplateBytes card.svg.tmpl 原始字节；目录里没有该文件传 null
     */
    static PlayDefinition load(String dirName, byte[] playYamlBytes, byte[] cardTemplateBytes) {
    ...
    private static Map<String, Object> parseYaml(byte[] bytes) {
        Object doc;
        try {
            // SafeConstructor：只构造 YAML 基本类型，杜绝 !!javax... 任意类实例化。
    ...
    private static LevelStep parseLevel(String var, Object levelObj) {
        ...
        for (int i = 0; i < thList.size(); i++) {
            ...
            if (last) {
                // 末项必须无 lt：兜底项，保证任何输入都有 label（全覆盖）。
                if (lt != null) {
                    throw new PlayLoadException(
                            "level('" + var + "') 末项必须省略 lt 作兜底");
                }
            ...
            for (Map.Entry<String, Object> ke : keys.entrySet()) {
                ...
                for (Map.Entry<String, Object> fe : fields.entrySet()) {
                    if (!(fe.getValue() instanceof String s) || s.length() > 120) {
                        throw new PlayLoadException(
                                "tables." + te.getKey() + "." + ke.getKey() + "." + fe.getKey()
                                        + " 必须为 ≤120 字符串");
                    }
    ...
    private static List<OutputField> parseOutput(Object outputObj) {
        ...
        for (Object o : fList) {
            ...
            if (!Set.of("number", "string", "object").contains(type)) {
                throw new PlayLoadException("output field '" + name + "' type 必须为 number/string/object");
            }
            if (!seen.add(name)) {
    ...
    /**
     * compute template 步骤的加载期 lint：card-only 内置变量（{@link #CARD_ONLY_BUILTINS}）
     * 在 compute ctx 里不存在，引用会让每次请求 500 —— 加载期直接拒。其余标识符可能是
     * SQL 列（运行时才知道），交给 fixtures 保证。
     */
    ...
    /**
     * SVG 模板启动 lint（设计定稿 §2.3）：
     *
     * <ol>
     *   <li>禁用子串（脚本 / DTD，{@link #TEMPLATE_FORBIDDEN}，防最常见注入面）
     *   <li>禁非 BMP 码位（emoji 等 —— 渲染环境只有 Noto Sans CJK，无 emoji 字体，
     *       Batik 也不支持彩色 emoji，必然渲染成 tofu 方框；emoji 请放 manifest 的
     *       {@code emoji:} 元数据字段）
     *   <li>DOM 级外部引用校验：模板必须是 well-formed XML；任何元素的
     *       {@code href}/{@code xlink:href}/{@code src} 属性只允许文档内 {@code #id}
     *       引用（substring 黑名单可被空白 / 单引号 / 非 http scheme 绕过 ——
     *       {@code href = "https://..."}、{@code file:///} 都在这里拦）
     *   <li>{@code url(...)} 引用（fill/filter/style 等）只允许 {@code #id}
     *   <li>每个 {@code ${var}} 必须能解析到 compute var / output 字段名 / 内置变量
     *       （点路径只校验根段）
     * </ol>
     *
     * <p>运行时另有 {@link PlayCardRenderer} 的 no-load Batik 安全策略兜底（双保险）。
     */
    private static void lintTemplate(
        ...
        while (m.find()) {
            ...
            if (!known.contains(rootSeg)) {
                throw new PlayLoadException(
                        "card.svg.tmpl 占位符 '${" + path + "}' 解析不到 compute var / output 字段 / 内置变量");
            }
        }
    }

    /**
     * 模板解析为 DOM（安全配置：禁 DTD / 禁外部实体 / 禁 XInclude），遍历全部元素属性，
     * {@code href}/{@code src}（任意 namespace，含 xlink）只允许 {@code #id} 文档内引用。
     * 解析失败（非 well-formed）同样拒载 —— fail closed。
     */
