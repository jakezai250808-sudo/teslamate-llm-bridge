...
用中文报告可操作的问题，说明触发条件、用户影响和最小相关代码位置；证据不足时说明不确定性，
不要把未运行的检查写成通过。聚焦本次变更造成的实际行为或安全回归，不重复格式检查。

- **只读 SQL 与车辆过滤**：玩法 SQL 必须保留真实的 `car_id = :car_id` 过滤、参数绑定和时间窗口，
  不得用注释或恒真表达式骗过校验，也不能将输入拼成任意 SQL。安全路径：沿用单条只读查询、超时和单行聚合约束；
  对外提供多用户访问时，车辆过滤不能代替入口鉴权与车辆授权，不凭传入 car ID 扩大可读范围。
- **样本门槛与评分可解释性**：样本不足必须保留未评分语义，阈值边界、兜底分支、输出字段与 fixtures 应一致，
  不得只改期望值或删除失败样本来掩盖公式错误。安全路径：按 manifest 手算预期并覆盖不同 level 与 unscored 分支；
  按既有规范对空集合聚合使用 `COALESCE` 是合法行为，不应套用客户端规则把所有聚合零值误判为未知。
- **LLM/图片输出与隐私**：不得把凭据、原始车辆轨迹或未授权个人数据写进提示词、外部服务请求、fixtures 或评审评论；
  模板和模型输出不能成为执行任意 SQL、shell 或车辆操作的授权。安全路径：仅输出玩法声明且用户授权的必要字段，
  示例使用合成数据，秘密保留在本地或服务端秘密配置，查询和生成失败不得伪装为真实计算结果。

这些规则补充而不替代现有玩法规范与验证流程；不配置云端审查开关、触发频率或模型，也不授予部署或合并权限。
